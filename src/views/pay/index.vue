<template>
  <div class="pay">
    ...
    <div class="address">
      ...
      <div class="info" v-if="myAddress">
        <div class="info-content">
          <span class="name">{{ myAddress.name }}</span>
          <span class="mobile">{{ myAddress.phone }}</span>
        </div>
    ...
    <div class="pay-list">
      <div v-for="item in goodsList" :key="item.goods_id">
        ...
        <div class="flow-num-box">
          <span>共 {{ item.total_num }} 件商品，合计：</span>
    ...
    <div class="footer-fixed">
      ...
      <div class="tipsbtn" @click="submitTheOrder">提交订单</div>
    </div>
  </div>
</template>
...
import { getOrder, submitOrder } from '@/api/order'
export default {
  name: 'PayIndex',
  data () {
    return {
  ...
  methods: {
    ...
    async getAddress () {
      ...
      console.log(list)
      this.myAddress = list[0]
    },
    // 获取订单详情
    async getOrderDet () {
      if (this.mode === 'cart') {
        const { data: { order, personal } } = await getOrder(this.mode, {
  ...
  computed: {
    longAddress () {
      return this.myAddress.region.province + this.myAddress.region.city + this.myAddress.region.region + this.myAddress.detail
    },
    mode () {
